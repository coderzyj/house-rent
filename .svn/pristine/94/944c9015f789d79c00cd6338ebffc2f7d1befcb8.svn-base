...
public class CustomScheduledJob extends AbstractScheduledJob {
	...
	private List<MasterData> loadMasterData(int index, String sysName) {
		...
		int count = list.size();
		logger.info("查询" + count + "条MasterData总耗时 : " + (end - begin)  + " ms");
		return list;
	}
	...
	private List<MasterData> insertMasterData(List<MasterData> list) {
		...
		if (dataList != null) {
			logger.info(list.size() + " 条中包括 " + dataList.size() + " 条已经导入了，需要删除。");
		}
		if (dataList != null && dataList.size() > 0) {
			int delCount = jobUtil.masterDataService.deleteMasterdata(dataList);
			logger.info(delCount + " 条已经被删除！！");
		}
		...
		Long end = new Date().getTime();
		logger.info("总计" + count + "条,插入" + ret + "条MasterData总耗时 : " + (end - begin)  + " ms");
		return retList;
	}
