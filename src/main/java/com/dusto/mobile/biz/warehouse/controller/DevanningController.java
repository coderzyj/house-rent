...
public class DevanningController {
	...
	public BaseResponse devanning(@RequestBody DevanningServiceRequest req) {
		logger.info("entry DevanningController:devanning,user:"+req.getuName()+" input parameters:" + JSON.toJSONString(req));
		...
		if (function == null) {
			...
			logger.info("exit DevanningController:devanning,user:"+req.getuName()+"  output parameters:" + JSON.toJSONString(res));
			return res;
		}
		...
		logger.info("entry DevanningController:devanning,user:"+req.getuName()+"  output parameters:ZFLAG=" + result + ",ZMESS=" + errMsg
